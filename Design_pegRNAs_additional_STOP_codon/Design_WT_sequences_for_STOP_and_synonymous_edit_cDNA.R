...
exon26 <- filter(exons, exons$Exon == 26)
exon26_start <- exon26[1,2]
exon26_end <- exon26[1,3]
exon28 <- filter(exons, exons$Exon == 28)
exon28_start <- exon28[1,2]
exon28_end <- exon28[1,3]

exon_26 <- str_sub(gene_seq,exon26_start, exon26_end)
exon_27 <- str_sub(gene_seq,exon27_start, exon27_end)
exon_28 <- str_sub(gene_seq,exon28_start, exon28_end)
exon26_exon27_exon28 <- paste(exon_26, exon_27, exon_28, sep = "")
...
## create input list additional nonsense mutations
input_list <- data.frame(matrix(ncol = 5, nrow = 0))
colnames(input_list) <- c("InputID","Codon_pos", "Codon_count", "MutationType", "InputSequence") 
exon_edit_subseq <- exon26_exon27_exon28
t = 0
counter = 1
codon_end = 184
codon_counter = 1
for(i in 185:394){ # starts two nucleotides after the beginning of exon 27 as exon 27 starts with an incomplete codon
  exon_edit_subseq <- exon26_exon27_exon28
  if(i > 394){
    break
  }
  else if(i <= 394 && counter == 3){
    counter = 1
    ...
    nt_2 <- codon_end - 1
    nt_3 <- codon_end 
    codon <- str_sub(exon26_exon27_exon28, nt_1, nt_3)
    if(codon %in% check_pos1){
      var <- get_stop_codon_pos1(codon)
      edit_pos <- nt_1 - 185 + 3 # nt_1 because edit is on first position; -183 + 3 to get position on exon 27 sequence only
      ...
      codon_after <- nt_3 + 3
      if(codon_after <= 394){
        codon_for_synonym_mut <- str_sub(exon26_exon27_exon28, nt_1 + 3, nt_3 + 3)
        if(codon_for_synonym_mut != "ATG" & codon_for_synonym_mut != "TGG" & codon_for_synonym_mut != "TGA" & codon_for_synonym_mut != "TAA" & codon_for_synonym_mut != "TAG"){
          ...
          edit_synonym <- str_c(ref_synonym)
          ...
          if(codon_before >= 185){
            codon_for_synonym_mut <- str_sub(exon26_exon27_exon28, nt_1 - 3, nt_1 - 1)
            if(codon_for_synonym_mut != "ATG" & codon_for_synonym_mut != "TGG" & codon_for_synonym_mut != "TGA" & codon_for_synonym_mut != "TAA" & codon_for_synonym_mut != "TAG"){
              var_synonym <- get_synonym_mutation(codon_for_synonym_mut)
              ref_synonym <- str_sub(codon_for_synonym_mut,3,3)
              edit_synonym <- str_c(ref_synonym)
      ...
      else if(codon_after > 395 & codon_before >= 185){
        codon_for_synonym_mut <- str_sub(exon26_exon27_exon28, nt_1 -3, nt_1 -1)
        if(codon_for_synonym_mut != "ATG" & codon_for_synonym_mut != "TGG" & codon_for_synonym_mut != "TGA" & codon_for_synonym_mut != "TAA" & codon_for_synonym_mut != "TAG"){
          var_synonym <- get_synonym_mutation(codon_for_synonym_mut)
          ref_synonym <- str_sub(codon_for_synonym_mut,3,3)
      ...
      edit_pos <- nt_2 - 185 + 3 # nt_2 because edit is on second position; -363 + 3 to get postion on exon 27 sequence only
      edit_pos_genome <- 29560020 + edit_pos -1
      ...
      edit_pos <- nt_3 - 185 + 3 # nt_3 because edit is on third position; -363 + 3 to get postion on exon 27 sequence only
      edit_pos_genome <- 29560020 + edit_pos -1
...
#write_xlsx(input_list, "WT_sequences_stop_cDNA.xlsx") 
...
codon_end = 184
codon_counter = 1
for(i in 185:394){ # starts two nucleotides after the beginning of exon 27 as exon 27 starts with an incomplete exon
  exon_edit_subseq <- exon26_exon27_exon28
  if(i > 394){
    break
  }
    ...
    if(var != "ATG" & var != "TGG" & var != "TGA" & var != "TAA" & var != "TAG"){
      ...
      if(codon_after <= 394){
        ...
        if(codon_for_synonym_mut != "ATG" & codon_for_synonym_mut != "TGG" & codon_for_synonym_mut != "TGA" & codon_for_synonym_mut != "TAA" & codon_for_synonym_mut != "TAG"){
          ...
          exon_edit_subseq <- str_sub(exon_edit_subseq, nt_3 - 10, nt_3 + count_char_edit + 2 + count_char_edit_synonym -1 + 10)
          t = t+1
          input_list_control[t,1] <- paste0("synonym_", edit_pos_genome)
          input_list_control[t,2] <- paste0("nucleotide_pos_", edit_pos)
          input_list_control[t,3] <- codon_counter
          input_list_control[t,4] <- "Nonsense mutation"
      ...
      else if(codon_after > 394 & codon_before >= 185){
        codon_for_synonym_mut <- str_sub(exon26_exon27_exon28, nt_1 -3, nt_1 -1)
        if(codon_for_synonym_mut != "ATG" & codon_for_synonym_mut != "TGG" & codon_for_synonym_mut != "TGA" & codon_for_synonym_mut != "TAA" & codon_for_synonym_mut != "TAG"){
          var_synonym <- get_synonym_mutation(codon_for_synonym_mut)
          ref_synonym <- str_sub(codon_for_synonym_mut,3,3)
        ...
        print("No edit to stop possible.")
        counter = 1
        i = i+1
      }
    }
    else if(var == "ATG" | var == "TGG" | var == "TGA" | var == "TAA" | var == "TAG"){
      print("Codon is ATG, TGG, TGA, TAA, TAG.")
      counter = 1
      i = i+1
    }
  }
  else if(i <= 394 && counter < 3){
    print("counter ist kleiner 3")
    counter = counter + 1
    i = i+1
  }
}
...
#write_xlsx(input_list_control, "WT_sequences_synonymous_edit_cDNA.xlsx") 

input_searching <- rbind(input_list[,c(1,3,5)], input_list_control[,c(1,3,5)])
write_xlsx(input_searching, "WT_sequences_stop_and_synonymous_edit_cDNA.xlsx")
