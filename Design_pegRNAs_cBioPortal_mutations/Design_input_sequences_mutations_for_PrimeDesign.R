...
#generate pegRNAs for mutations
...
print(length(unique(mutations_unique$`Sample ID`)))
...
## create input list for PrimeDesign based on mutation list from cBioPortal
input_list <- data.frame(matrix(ncol = 2, nrow = 0))
colnames(input_list) <- c("InputID", "InputSequence")   
  ...
  if(mutations_unique[i,9] == "SNP"){
    ...
    if(right_distance >= 4 && codon_pos == 33){
      ...
      if(marker_full_codon != "ATG" && marker_full_codon != "TGG" && marker_full_codon != "TGA" && marker_full_codon != "TAA" && marker_full_codon != "TAG"){
        ...
        marker_edit <- str_c("(", marker_ref, "/", marker_var,")")
        ...
        start_position_long <- start_position-300
        end_position_long <- end_pos_marker_edit+300+count_char_edit-count_char_ref+count_char_marker_edit-count_char_marker_ref
        ...
        if(left_distance >= 4){
          ...
          if(marker_full_codon != "ATG" && marker_full_codon != "TGG" && marker_full_codon != "TGA" && marker_full_codon != "TAA" && marker_full_codon != "TAG"){
            ...
            count_char_marker_edit <- nchar(marker_edit)
            count_char_marker_ref <- nchar(marker_ref)
            start_position_long <- start_pos_marker_edit-300
            end_position_long <- end_position+300+count_char_edit-count_char_ref+count_char_marker_edit-count_char_marker_ref
            gene_seq_edit <- substr(gene_seq_edit, start_position_long, end_position_long)
            input_list[i,2] <- gene_seq_edit
          }
          else if(marker_full_codon == "ATG" | marker_full_codon == "TGG" | marker_full_codon == "TGA" | marker_full_codon == "TAA" | marker_full_codon == "TAG"){
    ...
    if(codon_pos == 33){
      if(codon_pos_start == 33 && left_distance >= 4){
        ...
        if(marker_full_codon != "ATG" && marker_full_codon != "TGG" && marker_full_codon != "TGA" && marker_full_codon != "TAA" && marker_full_codon != "TAG"){
          ...
          text <- "Codon before delition is ATG, TGG, TAA, TAG or TGA."
          input_list[i,2] <- text
        }
      }
      else if(codon_pos_start == 33 && left_distance < 4){
        text <- "No full codon before deletion."
        input_list[i,2] <- text
      }
      ...
      else if(codon_pos_start == 0 && left_distance >= 3){
        start_pos_marker_edit <- start_position - 1
        end_pos_marker_edit <- start_pos_marker_edit
        marker_ref <- substr(exon_seq_marker, start_pos_marker_edit, end_pos_marker_edit)
    ...
    edit <- str_c("(+", var ,")")
    count_char_edit <- nchar(edit)
    stri_sub(gene_seq_edit, end_position, start_position) <- edit
    # find out on which codon position edit is and include marker edit
    exon <- exons[which_exon,]
    exon_start_pos <- exon[,2]
    exon_end_pos <- exon[,3]
    ...
    if(codon_pos_start == 0){
      if(left_distance >= 3){
        ...
        if(marker_full_codon != "ATG" && marker_full_codon != "TGG" && marker_full_codon != "TGA" && marker_full_codon != "TAA" && marker_full_codon != "TAG"){
          ...
          text <- "Codon before insertion is ATG, TGG, TAA, TAG or TGA."
          input_list[i,2] <- text
        }
      }
      else if(codon_pos_start == 0 && left_distance < 3){
        text <- "No full codon before insertion."
        input_list[i,2] <- text
      }
    }
    else if(codon_pos_start == 33){
      if(left_distance >= 4){
        start_pos_marker_edit <- start_position - 2
        end_pos_marker_edit <- start_pos_marker_edit
        marker_ref <- substr(exon_seq_marker, start_pos_marker_edit, end_pos_marker_edit)
...
write_xlsx(exon27_mutations, "Exon_27_PrimeDesign_Input_mutation.xlsx") 
